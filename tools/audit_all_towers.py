...
{let {s,t}=reset('ranger',11),range=A.towerStats(t).range,stealth=spawn('stealth',t.x+range*.95,t.y);stealth.stealth=true;ok('Ranger L11 Adaptive Rounds grants full stealth detection',A.canSee(t,stealth,range));let armored=spawn('armored',t.x+60,t.y),normal=spawn('grunt',t.x+60,t.y),fast=spawn('runner',t.x+60,t.y),st=A.towerStats(t),pa=A.rangerRoundProfile(t,armored,st,100,true),pn=A.rangerRoundProfile(t,normal,st,100,true),pf=A.rangerRoundProfile(t,fast,st,100,true);ok('Ranger L11 Adaptive Rounds increases armor penetration against armored targets',pa.armorPen>st.armorPen,{base:st.armorPen,adaptive:pa.armorPen});ok('Ranger L11 Adaptive Rounds increases damage against fast targets',pf.damage>pn.damage*1.1,{normal:pn,fast:pf});}
{let {s,t}=reset('ranger',15),e=spawn('grunt',t.x+60,t.y);t.shots=4;s.rng=1;forceFire(t,e);let p=s.projectiles[0];ok('Ranger L15 Combat Mastery enhances every fifth primary attack',p?.rangerMastery===true&&p.damage>A.towerStats(t).damage,{projectile:p});ok('Ranger L15 Combat Mastery has distinct VFX',(s.effects||[]).some(x=>x.kind==='rangerMastery'));}
...
{let {t}=reset('ranger',15);A.openSelection(t);let txt=document.querySelector('#selMilestone').textContent;ok('Tower inspection shows all unlocked milestones',towersByText=['Sharpened Rounds','Rapid Cycling','Double Tap','Adaptive Rounds','Combat Mastery'].every(x=>txt.includes(x)),txt);}
// REPEATER
...
{let {s,t}=reset('railgun',15),primary=spawn('grunt',t.x+40,t.y,1),st=A.towerStats(t);for(let i=0;i<7;i++)spawn('grunt',t.x+70+i*25,t.y,1);forceFire(t,primary);settleProjectiles();ok('Railgun L15 Hypervelocity kills do not consume pierce capacity',t.kills>st.pierce,{kills:t.kills,pierce:st.pierce});}
// FROST
...
{let {s,t}=reset('hive',5),e1=spawn('grunt',t.x+70,t.y);A.tick(.02);let d=t.drones[0];d.cool=1;d.targetId=e1.id;e1.dead=true;let e2=spawn('grunt',t.x+75,t.y),before=t.shots;A.tick(1/60);let refunded=d.cool;ok('Hive L5 Faster Recall immediately reacquires after target loss',d.targetId===e2.id&&d.recallFlash>s.time,{targetId:d.targetId,expected:e2.id,drone:d});ok('Hive L5 Faster Recall refunds most of remaining cooldown',refunded>0&&refunded<.5,{refunded});for(let i=0;i<30&&t.shots===before;i++)A.tick(1/60);ok('Hive L5 Faster Recall fires earlier after reacquisition',t.shots>before,{before,after:t.shots,drone:d});}
